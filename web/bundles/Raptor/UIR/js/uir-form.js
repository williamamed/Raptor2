...
(function($) {
    UIR.ns('UIR.Components');

    UIR.Components.Form = function(element, options) {
        this.element = element;
        this.options = options;
        this.init();
    }

    UIR.Components.Form.prototype = {
        constructor: UIR.Components.Form,
        ...
        prepareOpts: function() {
            ...
            if (this.options.onSubmit && typeof this.options.onSubmit == 'function'){
                $(this.element).on('form.success',$.proxy(this.options.onSubmit,this))
               
            }
            if (this.options.onError && typeof this.options.onError == 'function')
                $(this.element).on('form.error',$.proxy(this.options.onError,this))
            ...
            $(this.element).find('[uir-custom][name]').each(function() {
                
                if ($(this).data($(this).attr('uir-custom')))
                    me.customFields[$(this).attr('name')]=($(this).data($(this).attr('uir-custom')));
            })
        ...
        validCustom: function() {
            var result = true;
            for (var field in this.customFields) {
                
                if (this.customFields[field].validate && !this.customFields[field].validate()){
        ...
        validar: function() {
            ...
            var validation = $(this.element).valid();
            if (this.validCustom() && validation) {
                return true;
            } else
                return false;
        ...
        setValue: function(/*campo,valor*/) {
            ...
            if(this.customFields[arguments[0]] && this.customFields[arguments[0]].setValue)
                this.customFields[arguments[0]].setValue(text)
        ...
        setValues: function() {
            ...
            for (var campo in result) {
                ...
                $($this).find('textarea[name="' + campo + '"]').each(function() {
                    ...
                    if (this.customFields[campo] && this.customFields[campo].setValue) {
                        this.customFields[campo].setValue(result[campo]);
                    }
        ...
        reset: function() {
            ...
            $(this.element).find('select').val('').removeClass('error');
            $(this.element).find('textarea').each(function() {
                $(this).val('');
                $(this).removeClass('error');
            })
            for (var field in this.customFields) {
                if (this.customFields[field].reset)
                    this.customFields[field].reset()
        ...
        submit: function() {
            ...
            if (params.success && typeof params.success == 'function')
                $(this.element).on('form.success',$.proxy(params.success,this));
            
            
            if (params.error && typeof params.error == 'function')
                $(this.element).on('form.error',$.proxy(params.error,this));
            ...
            if (this.validar()) {
                ...
                var html = $.ajax({
                    ...
                    url: url,
                    async: false,
                    success: $.proxy(this.onSubmit,this),
        ...
        onSubmit: function(data) {
            var evt=$.Event('form.success');
            $(this.element).trigger(evt,[data]);
        },
        onError: function(data) {
            var evt=$.Event('form.error');
            $(this.element).trigger(evt,[data])
        },
        getFields: function(field) {
            ...
            if (field) {
                  if (this.customFields[field] && this.customFields[field].getValue) {
                        return this.customFields[field].getValue();
                 }
                ...
                for (var name in this.customFields) {
                    if (this.customFields[name].getValue)
                       obj.push({name: name, value: this.customFields[name].getValue()});
    ...
    $.fn.FormUIR= $.fn.formUIR = function(option) {
        if (typeof option === 'string') {
            if(option === 'data')
                return $(this).data('FormUIR');
            var obj=$(this).data('FormUIR');
            ...
            if(obj[option])
                return obj[option].apply(obj, arg);
        } else
            $(this).each(function() {
                var $this = new UIR.Components.Form(this, option);
                $(this).data('FormUIR', $this)
            })
